<?php
class MovePblSessionResourcesFix {
    ...
    private $updateLkProblemResourceFormat = "UPDATE lk_resource SET type_id = %d,resource_type_id = %d, resource_type='mediabank',type ='ta' WHERE auto_id = %d;";
    ...
    */
    private function processProblemResources($pblId, $pblsession1) {
        ...
        $adapter = $mediabankResource->getAdapter();
        $query = <<<QUERY
select * from lk_resource where type = 'pbl' and type_id = {$pblId} and resource_type is null and resource_type_id in (4,5);
QUERY;
        $pblResources = $adapter->query($query)->fetchAll();        
        if(!empty($pblResources)) {
            foreach($pblResources as $pblResource) {
                ...
                if($newMediabankTitle !== false) {
                    ...
                    $this->problem['mediabanktitlefalse'][] =  'MEDIABANKTITLE IS FALSE '. $pblResource['auto_id'];
                }
    ...
    private function getNewMediabankTitleForProblemResources($mediabankTitle, &$pblResource) {
        if(empty($mediabankTitle)) {
            ...
            } else if($pblResource['resource_type_id'] == 4) {
                return 'Tutor Guide';
            } else if($pblResource['resource_type_id'] == 5) {
                return 'Student Guide';
            }
        } else if(stripos($mediabankTitle,'Problem Summary') !== false) {
            return 'Problem Summary';
        } else if(stripos($mediabankTitle,'Tutor Guide') !== false) {
            return 'Tutor Guide';
        } else if(stripos($mediabankTitle,'Medical Humanities') !== false) {
            return 'Medical Humanities';
        } else if(stripos($mediabankTitle,'Mechanism') !== false) {
            return 'Mechanism';
        } else if(stripos($mediabankTitle,'Student Guide') !== false) {
            return 'Student Guide';
        }
        return false;
    }
