 ...
 *
 */
class PblResourceType extends Zend_Db_Table_Abstract {
    protected $_name = 'lk_resourcetype';

    public function getAllPblResourceType() {
        try {
            $rows = $this->fetchAll('parent_id = 0', 'auto_id ASC');
            $result = array();
            foreach ($rows as $row) {
                 $result[$row->auto_id] = trim($row->pbl_resource_type);
            }
            asort($result);
            return $result;
        } catch (Exception $ex) {
    ...
    public function getAllPblResourceAutoidAndSingleOrMulti() {
        try {
            $rows = $this->fetchAll(NULL, 'auto_id ASC');
            $result = array();
            foreach ($rows as $row) {
                 $resourceType = trim($row->pbl_resource_type);
