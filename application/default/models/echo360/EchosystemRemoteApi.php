 ...
 * 
 * Modified to integrate it with Zend Framework 
 * Added function generate_presentation_url()
 * @author  Kamal Soni
 *  
 */
...
class EchosystemRemoteApi {
     ...
     * @param string - realm
     */
    public function __construct($baseurl, $consumerkey, $consumersecret, $realm) {
        if ($baseurl != null) {
    ...
    /**
     * Generate a presenation redirect URL
     * 
     ...
     * @return string $url
     */
    public function generate_presentation_url($username, $presentation_id, $media = null, $is_instructor = true) {
        $echocenterurl  = $this->baseurl . 'ess/echo/presentation/'. $presentation_id;
        if(! is_null($media)) {
            if(in_array($media, array('m4v', 'mp3'))) {
                $echocenterurl .= '/mediacontent.'.$media;
            }
        }
        $apiurl         = $this->baseurl . 'ess/personapi/v1/' . urlencode($username) . '/session';
        $apiparams      = array(
                                'redirecturl'       => $echocenterurl,
                                'instructor'        => (($is_instructor === true) ? 'true' : 'false'),
